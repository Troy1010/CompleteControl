...
bool Cmd_DisableKey2_Execute(ParamInfo * paramInfo, void * arg1, TESObjectREFR * thisObj, TESObjectREFR* contObj, Script * scriptObj, ScriptEventList * eventList, double * result, UInt32 * opcodeOffsetPtr)
{
	...
	if (!ExtractArgsEx(paramInfo, arg1, opcodeOffsetPtr, scriptObj, eventList, &iDXScancode)) { Logd(std::string(__func__) + "`Failed arg extraction"); return false; }
	auto pControl = Controls.FindByScancode(iDXScancode);
	if (!pControl) { Logd(std::string(__func__) + "`Could not retrieve pControl with iDXScancode:" + TMC::ToLogStr(iDXScancode)); return true; }
...
void DisableControl_Helper(UInt32 iControlID, UInt8 iModIndex)
{
	...
	if (!pControl) { Logv(std::string(__func__) + "`Received unregistered ControlID:" + TMC::ToLogStr(iControlID)); return; }
...
bool Cmd_DisableControl2_Execute(COMMAND_ARGS)
{
	Logd(std::string(__func__) + "`Open");
	UInt32	iControlID = 0;
	if (!ExtractArgsEx(paramInfo, arg1, opcodeOffsetPtr, scriptObj, eventList, &iControlID)) { Logd(std::string(__func__) + "`Failed arg extraction"); return false; }
...
bool Cmd_DisableControl_ByRef_Execute(COMMAND_ARGS)
{
	...
	if (!ExtractArgsEx(paramInfo, arg1, opcodeOffsetPtr, scriptObj, eventList, &vControlID_Form)) { Logd(std::string(__func__) + "`Failed arg extraction"); return false; }
...
bool Cmd_DisableControls_Execute(COMMAND_ARGS)
{
	Logd(std::string(__func__) + "`Open");
	UInt32 arrID = 0;
	if (!ExtractArgsEx(paramInfo, arg1, opcodeOffsetPtr, scriptObj, eventList, &arrID)) { Logd(std::string(__func__) + "`Failed arg extraction"); return false; }
			...
			if (g_arrayIntfc->GetElements(arr, elems, keys)) {
				...
				for (UInt32 i = 0; i < size; i++) {
					if (elems[i].GetType() == OBSEElement::kType_Numeric) {
						...
						Logv(std::string(__func__) + "`Disabling:" + TMC::ToLogStr((UInt32)elems[i].Form()->refID));
						DisableControl_Helper(elems[i].Form()->refID, iModIndex);
					}
					else {
						Logd(std::string(__func__) + "`Received invalid type for ControlID");
					}
				}
			}
			...
			if (g_arrayIntfc->GetElements(arr, elems, keys)) {
				...
				for (UInt32 i = 0; i < size; i++) {
					if (elems[i].GetType() == OBSEElement::kType_Numeric) {
						...
						Logv(std::string(__func__) + "`Enabling:" + TMC::ToLogStr((UInt32)elems[i].Form()->refID));
...
bool Cmd_RegisterControl_Execute(COMMAND_ARGS)
{
	...
	if (!ExtractArgsEx(paramInfo, arg1, opcodeOffsetPtr, scriptObj, eventList, &vControlID_Form, &dxScancode, &iMenuModeType)) { Logd(std::string(__func__) + "`Failed arg extraction"); return false; }
...
{
	auto pControl = Controls.FindByID(iControlID);
	if (!pControl) { Logv(std::string(__func__) + "`Received unregistered ControlID:" + TMC::ToLogStr(iControlID)); return false; }
...
bool Cmd_IsDisabled_Execute(COMMAND_ARGS)
{
	Logd(std::string(__func__) + "`Open");
	UInt32	vControlID = 0;
	if (!ExtractArgsEx(paramInfo, arg1, opcodeOffsetPtr, scriptObj, eventList, &vControlID)) { Logd(std::string(__func__) + "`Failed arg extraction"); return false; }
...
{
	auto pControl = Controls.FindByID(iControlID);
	if (!pControl) { Logv(std::string(__func__) + "`Received unregistered ControlID:" + TMC::ToLogStr(iControlID)); return 0xFF; }
...
{
	Logv(std::string(__func__) + "`Open. scriptObjRefID:"+ TMC::ToLogStr(scriptObj->refID));
	UInt32	iControlID = -1;
	if (!ExtractArgsEx(paramInfo, arg1, opcodeOffsetPtr, scriptObj, eventList, &iControlID)) { Logd(std::string(__func__) + "`Failed arg extraction"); return false; }
